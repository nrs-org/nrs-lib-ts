...
export class DAH_standards {
    ...
    #emotionVector(
        ...
        const combinedFactor = combinePow(
            contribFactors,
    ...
    consumed(
        ...
        const boredomScore = boredom * baseScore *
            Math.pow(ratio, Boredom.factorWeight);
    ...
    visual(
        ...
        const visualScore = ((base * (unique + 2.0)) / 3.0) *
            visualType.factor *
            2.0;
    ...
    writing(
        ...
        const M = 4.0;
        const baseScore = M * (characterComplexity + storyQuality) / 2 *
            (1 + pacing) / 2 * (1 + originality) / 2;
        return {
            contributors,
            score: vector(context, [[AL, baseScore]]),
            DAH_meta: this.#impactMeta({
                name: "writing",
                writingArgs: {
                    characterComplexity,
...
export type IRName = ImpactName | RelationName;
